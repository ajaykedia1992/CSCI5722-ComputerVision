...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    #############################################################################
    dict_scores = probs
    dict_scores[np.arange(N), y] -= 1.0
    dict_scores = dict_scores / float(N)

    db2 = np.sum(dict_scores, axis=0)

    dh1 = np.dot(dict_scores, W2.T)

    dW2 = np.dot(h1.T, dict_scores) + reg * W2
