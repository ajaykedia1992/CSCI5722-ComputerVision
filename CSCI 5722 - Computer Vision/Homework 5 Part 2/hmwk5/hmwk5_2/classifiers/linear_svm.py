...
def svm_loss_naive(W, X, y, reg):
  ...
  # to be an average instead so we divide by train.
  loss /= train
  dW /= float(train)
...
def svm_loss_vectorized(W, X, y, reg):
  ...
  margins[np.arange(train), y] = 0.0
  loss = np.sum(margins) / float(train)
  ...
  mask[np.arange(y.shape[0]), y] = - np.sum(mask, axis=1)
  dW = np.dot(X.T, mask)
  dW = dw / float(train)
  dW = dw + reg * W
  #############################################################################
